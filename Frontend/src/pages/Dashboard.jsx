...
const Dashboard = () => {
  ...
  const cards = [
    { label: "Total Videos", value: stats.totalVideos, icon: Film, color: "text-blue-500 bg-blue-500/10" },
    { label: "Subscribers", value: stats.totalSubscribers, icon: Users, color: "text-red-500 bg-red-500/10" },
    { label: "Total Likes", value: stats.totalLikes, icon: Heart, color: "text-amber-500 bg-amber-500/10" },
  ];
  ...
  return (
    <div className="w-full space-y-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Link
          to="/upload"
          className="inline-flex items-center gap-2 rounded-full bg-red-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm shadow-red-500/20 hover:bg-red-700 active:scale-95 transition self-start sm:self-auto"
        >
          <Upload size={15} />
      ...
      {statsLoading ? (
        <div className="grid gap-4 sm:grid-cols-3">
          {Array.from({ length: 3 }, (_, index) => (
            <div key={index} className="h-28 animate-pulse rounded-3xl bg-slate-200/70 dark:bg-zinc-800/80" />
          ))}
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-3">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-slate-600 dark:text-zinc-500">
              ({videosData?.length || 0})
            </span>
          </div>
        </div>
        ...
        {videosLoading ? (
          ...
          <div className="flex min-h-60 flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-900/30 p-8 text-center">
            <Film size={28} className="text-slate-500 dark:text-zinc-500 mb-2" />
            ...
            <Link
              to="/upload"
              className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-red-600 px-4 py-2 text-xs font-semibold text-white shadow-sm shadow-red-500/20 hover:bg-red-700 active:scale-95 transition cursor-pointer"
            >
